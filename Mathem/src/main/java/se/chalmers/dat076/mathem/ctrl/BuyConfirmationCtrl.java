...
public class BuyConfirmationCtrl {
    ...
    public void confirm() throws IOException{
        ...
        if(buyConBB.getSaveInfo()){
            System.out.println("Sparar info");
            Account a= new Account(buyConBB.getCardnumber());
            a.setCvc(buyConBB.getCvc());
            a.setName(buyConBB.getName());
            a.setExpdate(buyConBB.getExpdate());
            shop.getAccountCatalogue().create(a);
            
            Payswith pW = new Payswith(buyConBB.getCustomer().getName());
            pW.setCustomer1(buyConBB.getCustomer());
            pW.setAccount(a);
            shop.getPaysWithCatalogue().create(pW);
        }
